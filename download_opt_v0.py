        ...
        def segment_by_event(data, current_signal, settings, base_name, norm, fs, band):
            ...
            for cond in settings['segmentation']['selected_conditions']:
                if cond == 'null':
                    ...
                    for evt in settings['segmentation']['selected_events']:
                        evt_key = data.marks.app_settings['segmentation']['events'][evt]['label']
                        segments = []
                        for i in range(0, len(idx), 2):
        ...
        for i, file in enumerate(selected_files):
            try:
                ...
                if settings_dic['parameters'].get('band_segmentation'):
                    ...
                    for band in selected_bands:
                        ...
                        if settings_dic['preprocessing'].get('apply_preprocessing'):
                            ...
                            current_signal = current_band_signal

                            preprocessed_data = deepcopy(data)
                            save_outputs(preprocessed_data, base_name, band_name, 'prep')
                        else:
                        ...
                        if segmentation_type == 'condition':
                            segment_by_condition(data, current_signal, settings_dic, base_name, norm,
                                                 band=band_name)

                        elif segmentation_type == 'event':
                            segment_by_event(data, current_signal, settings_dic, base_name, norm, fs,
                                             band=band_name)
                ...
                # ---------------------------------      BROADBAND       --------------------------------
                    ...
                    preprocessed_data = deepcopy(data)
                    save_outputs(preprocessed_data, base_name, 'broadband', 'prep')
                    ...
                    if segmentation_type == 'condition':
                        segment_by_condition(data, current_signal, settings_dic, base_name, norm, band=None)

                    elif segmentation_type == 'event':
                        segment_by_event(data, current_signal, settings_dic, base_name, norm, fs, band=None)
