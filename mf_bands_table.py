...
class MFBandTableWidget(QtWidgets.QTableWidget):
...
class MFBandTable(QtWidgets.QDialog):
    def __init__(self, parameters_widget=None):
        ...
        self.mfbandsTable = MFBandTableWidget(self)
        self.mfbandsTable.setObjectName("bandsTable")
        ...
        if layout:
            ...
            layout.insertWidget(index, self.mfbandsTable)
    ...
    def _setup_table(self):
        ...
        self.mfbandsTable.setRowCount(0)
        self.mfbandsTable.setColumnCount(5)
        self.mfbandsTable.setHorizontalHeaderLabels(["Enabled", "Name", "Min. Freq.", "Max. Freq.", "Remove"])

        # Cabecera en negrita
        font = QFont()
        font.setBold(True)
        for i in range(5):
            item = self.mfbandsTable.horizontalHeaderItem(i)
            item.setFont(font)

        # Permitir reordenar filas
        self.mfbandsTable.setDragDropMode(QtWidgets.QAbstractItemView.InternalMove)
        self.mfbandsTable.setDragDropOverwriteMode(False)
        self.mfbandsTable.setSelectionBehavior(QtWidgets.QTableWidget.SelectRows)
        self.mfbandsTable.setDefaultDropAction(Qt.MoveAction)

        # # Eliminar widgets embebidos de la tabla
        for row in range(self.mfbandsTable.rowCount()):
            for col in range(self.mfbandsTable.columnCount()):
                widget = self.mfbandsTable.cellWidget(row, col)
                if widget:
                    widget.deleteLater()
        self.mfbandsTable.clearContents()
        self.mfbandsTable.setRowCount(0)
    ...
    def _add_band_row(self, name="custom", min_freq=0.0, max_freq=0.0):
        row = self.mfbandsTable.rowCount()
        self.mfbandsTable.insertRow(row)

        # Enabled
        checkbox = QtWidgets.QCheckBox()
        checkbox.setChecked(False)
        self.mfbandsTable.setCellWidget(row, 0, self._center_widget(checkbox))

        # Name
        name_item = QtWidgets.QTableWidgetItem(name)
        name_item.setTextAlignment(Qt.AlignCenter)
        self.mfbandsTable.setItem(row, 1, name_item)

        # Min freq
        min_item = QtWidgets.QTableWidgetItem(f"{float(min_freq):.1f}")
        min_item.setTextAlignment(Qt.AlignCenter)
        self.mfbandsTable.setItem(row, 2, min_item)

        # Max freq
        max_item = QtWidgets.QTableWidgetItem(f"{float(max_freq):.1f}")
        max_item.setTextAlignment(Qt.AlignCenter)
        self.mfbandsTable.setItem(row, 3, max_item)
        ...
        remove_button.clicked.connect(lambda _, r=row: self.mfbandsTable.removeRow(r))
        self.mfbandsTable.setCellWidget(row, 4, self._center_widget(remove_button))

    def _reset_table(self):
        # try:
        #     self.mfbandsTable.cellChanged.disconnect()
        # except TypeError:
        #     pass
        self._setup_table()
        # self.mfbandsTable.cellChanged.connect()
    ...
    def _accept_and_close(self):
        ...
        for row in range(self.mfbandsTable.rowCount()):
            checkbox_container = self.mfbandsTable.cellWidget(row, 0)
            ...
            name_item = self.mfbandsTable.item(row, 1)
            min_item = self.mfbandsTable.item(row, 2)
            max_item = self.mfbandsTable.item(row, 3)
        ...
        if self.parameters_widget:
            self.parameters_widget.update_band_mf_label(self.accepted_mf_bands)
        self.close()
