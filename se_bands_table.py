...
class SEBandTableWidget(QtWidgets.QTableWidget):
...
class SEBandTable(QtWidgets.QDialog):
    def __init__(self, parameters_widget=None):
        ...
        self.sebandsTable = SEBandTableWidget(self)
        self.sebandsTable.setObjectName("bandsTable")
        ...
        if layout:
            ...
            layout.insertWidget(index, self.sebandsTable)
    ...
    def _setup_table(self):
        ...
        self.sebandsTable.setRowCount(0)
        self.sebandsTable.setColumnCount(5)
        self.sebandsTable.setHorizontalHeaderLabels(["Enabled", "Name", "Min. Freq.", "Max. Freq.", "Remove"])

        # Cabecera en negrita
        font = QFont()
        font.setBold(True)
        for i in range(5):
            item = self.sebandsTable.horizontalHeaderItem(i)
            item.setFont(font)

        # Permitir reordenar filas
        self.sebandsTable.setDragDropMode(QtWidgets.QAbstractItemView.InternalMove)
        self.sebandsTable.setDragDropOverwriteMode(False)
        self.sebandsTable.setSelectionBehavior(QtWidgets.QTableWidget.SelectRows)
        self.sebandsTable.setDefaultDropAction(Qt.MoveAction)

        # # Eliminar widgets embebidos de la tabla
        for row in range(self.sebandsTable.rowCount()):
            for col in range(self.sebandsTable.columnCount()):
                widget = self.sebandsTable.cellWidget(row, col)
                if widget:
                    widget.deleteLater()
        self.sebandsTable.clearContents()
        self.sebandsTable.setRowCount(0)
    ...
    def _add_band_row(self, name="custom", min_freq=0.0, max_freq=0.0):
        row = self.sebandsTable.rowCount()
        self.sebandsTable.insertRow(row)

        # Enabled
        checkbox = QtWidgets.QCheckBox()
        checkbox.setChecked(False)
        self.sebandsTable.setCellWidget(row, 0, self._center_widget(checkbox))

        # Name
        name_item = QtWidgets.QTableWidgetItem(name)
        name_item.setTextAlignment(Qt.AlignCenter)
        self.sebandsTable.setItem(row, 1, name_item)

        # Min freq
        min_item = QtWidgets.QTableWidgetItem(f"{float(min_freq):.1f}")
        min_item.setTextAlignment(Qt.AlignCenter)
        self.sebandsTable.setItem(row, 2, min_item)

        # Max freq
        max_item = QtWidgets.QTableWidgetItem(f"{float(max_freq):.1f}")
        max_item.setTextAlignment(Qt.AlignCenter)
        self.sebandsTable.setItem(row, 3, max_item)
        ...
        remove_button.clicked.connect(lambda _, r=row: self.sebandsTable.removeRow(r))
        self.sebandsTable.setCellWidget(row, 4, self._center_widget(remove_button))

    def _reset_table(self):
        # try:
        #     self.sebandsTable.cellChanged.disconnect()
        # except TypeError:
        #     pass
        self._setup_table()
        # self.sebandsTable.cellChanged.connect()
    ...
    def _accept_and_close(self):
        ...
        for row in range(self.sebandsTable.rowCount()):
            checkbox_container = self.sebandsTable.cellWidget(row, 0)
            ...
            name_item = self.sebandsTable.item(row, 1)
            min_item = self.sebandsTable.item(row, 2)
            max_item = self.sebandsTable.item(row, 3)
        ...
        if self.parameters_widget:
            self.parameters_widget.update_band_se_label(self.accepted_se_bands)
        self.close()
