...
from experiments.ecg_features.leads.view import LeadsWidget
...
def on_next_click(controller):
    ...
    current_idx = controller.view.stackedWidget.currentIndex() # idx is the index of the previous widget

    # Call the specific on_next_clicked function of the previous widget
    current_widget = controller.view.stackedWidget.widget(current_idx)
    ...
    controller.update_progressbar(current_idx)
    controller.view.stackedWidget.setCurrentIndex(current_idx)
    controller.view.backButton.setVisible(current_idx > 0)
    controller.view.nextButton.setVisible(current_idx > 0)
    # Go the next widget, set text to "Next"
    if current_idx < controller.view.total_steps:
        controller.view.nextButton.setText("Next")
    # If going to the last widget
    elif current_idx == controller.view.total_steps:
        controller.view.nextButton.setText("Run pipeline")
    # If in the last widget, close the app
    elif current_idx == controller.view.total_steps + 1:
...
def on_back_click(controller):
    ...
    """
    current_idx = controller.view.stackedWidget.currentIndex() # idx is the index of the previous widget
    current_idx -= 1 # Now is the index of the future widget

    # If going back to the LeadsWidget, set from_back to True to avoid reloading, sorry for this but I cannot find a better way :(
    if isinstance(controller.view.stackedWidget.widget(current_idx), LeadsWidget):
        controller.view.stackedWidget.widget(current_idx).controller.from_back = True
    ...
    controller.view.nextButton.setText("Run pipeline" if current_idx == controller.view.total_steps else "Next")
    controller.view.nextButton.setVisible(current_idx > 0)
