...
from experiments.eeg_features.segmentation import marks_utils
...
class SegmentationController:
    def __init__(self, ui):
        ...
        # Define variables
        idx_files_widget = next((i for i, d in enumerate(self.view.main_window.controller.experiment['pipeline']) if
                                 d.get('widget') == "FilesWidget"))
        self.files = self.view.main_window.stackedWidget.widget(idx_files_widget).controller.selected_files
    ...
    def update_max_samples(self):
        ...
        idx_files_widget = next((i for i, d in enumerate(self.view.main_window.controller.experiment['pipeline']) if
                                 d.get('widget') == "FilesWidget"))
        if self.view.conditionRButton.isChecked():
            max_samples = (self.view.trialBox.value()/1000) * self.view.main_window.stackedWidget.widget(idx_files_widget).controller.biosignal_info['fs']
        else:
            max_samples = -self.view.winBox_1.value() + self.view.winBox_2.value()
            max_samples = (max_samples/1000) * self.view.main_window.stackedWidget.widget(idx_files_widget).controller.biosignal_info['fs']
    ...
    def on_resample_toggle(self, checked):
        ...
        preprocessing = self.view.main_window.stackedWidget.widget(idx_preprocessing_widget)
