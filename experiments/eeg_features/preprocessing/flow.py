...
def get_preprocessing_config(controller):
    ...
    idx_files_widget = next((i for i, d in enumerate(controller.view.main_window.controller.experiment['pipeline']) if
                             d.get('widget') == "FilesWidget"))
    config = {
        "fs": controller.view.main_window.stackedWidget.widget(idx_files_widget).controller.biosignal_info['fs'],
...
def on_next_click(view):
    ...
    idx_segmentation_widget = next((i for i, d in enumerate(view.main_window.controller.experiment['pipeline']) if
                             d.get('widget') == "SegmentationWidget"))
    idx_files_widget = next((i for i, d in enumerate(view.main_window.controller.experiment['pipeline']) if
                             d.get('widget') == "FilesWidget"))
    idx_parameters_widget = next((i for i, d in enumerate(view.main_window.controller.experiment['pipeline']) if
                             d.get('widget') == "ParametersWidget"))
    segmentation = view.main_window.stackedWidget.widget(idx_segmentation_widget)
    # Initialize the segmentation widget
    files_controller_widget = view.main_window.stackedWidget.widget(idx_files_widget).controller
    ...
    if view.bandCBox.isChecked():
        ...
        view.main_window.stackedWidget.widget(idx_parameters_widget).controller.view.rpLabel.setText('Using bands from band segmentation step - ' + text)
