...
from experiments.eeg_features.bands_table import BandTableWidget
from experiments.eeg_features.preprocessing.flow import reset_all_controls
...
class PreprocessingController:
    ...
    def on_bandpass_toggle(self, checked):
        ...
        idx_files_widget = next(
            (i for i, d in enumerate(self.view.main_window.controller.experiment['pipeline']) if
             d.get('widget') == "FilesWidget"))
        ...
        if not checked:
            ...
            self.view.maxbroadBox.setValue(self.view.main_window.stackedWidget.widget(idx_files_widget).controller.biosignal_info['fs']/2)
    ...
    def on_band_filtering_toggle(self, checked):
        ...
        self.band_editor = None
        # Reset the RP
        idx_parameters_widget = next(
            (i for i, d in enumerate(self.view.main_window.controller.experiment['pipeline']) if
             d.get('widget') == "ParametersWidget"))
        self.view.main_window.stackedWidget.widget(idx_parameters_widget).rpCBox.setChecked(False)
    ...
    def on_show_event(self):
        if not self.first_show:
            ...
            idx_files_widget = next((i for i, d in enumerate(self.view.main_window.controller.experiment['pipeline']) if d.get('widget') == "FilesWidget"))
            files_widget_controller = self.view.main_window.stackedWidget.widget(idx_files_widget).controller
    ...
    def update_filter_plot(self, filter_type):
        ...
        idx_files_widget = next((i for i, d in enumerate(self.view.main_window.controller.experiment['pipeline']) if d.get('widget') == "FilesWidget"))
        fs = self.view.main_window.stackedWidget.widget(idx_files_widget).controller.biosignal_info['fs'] # widget(0) is the file selection widget
        b = firwin(
            numtaps,
            [low, high],
