...
class RPBandTableWidget(QtWidgets.QTableWidget):
...
class RPBandTable(QtWidgets.QDialog):
    def __init__(self, parameters_widget=None):
        ...
        self.rpbandsTable = RPBandTableWidget(self)
        self.rpbandsTable.setObjectName("bandsTable")
        ...
        if layout:
            ...
            layout.insertWidget(index, self.rpbandsTable)
    ...
    def _setup_table(self):
        ...
        self.rpbandsTable.setRowCount(0)
        self.rpbandsTable.setColumnCount(5)
        self.rpbandsTable.setHorizontalHeaderLabels(["Enabled", "Name", "Min. Freq.", "Max. Freq.", "Remove"])

        # Cabecera en negrita
        font = QFont()
        font.setBold(True)
        for i in range(5):
            item = self.rpbandsTable.horizontalHeaderItem(i)
            item.setFont(font)

        # Permitir reordenar filas
        self.rpbandsTable.setDragDropMode(QtWidgets.QAbstractItemView.InternalMove)
        self.rpbandsTable.setDragDropOverwriteMode(False)
        self.rpbandsTable.setSelectionBehavior(QtWidgets.QTableWidget.SelectRows)
        self.rpbandsTable.setDefaultDropAction(Qt.MoveAction)

        # # Eliminar widgets embebidos de la tabla
        for row in range(self.rpbandsTable.rowCount()):
            for col in range(self.rpbandsTable.columnCount()):
                widget = self.rpbandsTable.cellWidget(row, col)
                if widget:
                    widget.deleteLater()
        self.rpbandsTable.clearContents()
        self.rpbandsTable.setRowCount(0)
    ...
    def _add_band_row(self, name="custom", min_freq=0.0, max_freq=0.0):
        row = self.rpbandsTable.rowCount()
        self.rpbandsTable.insertRow(row)

        # Enabled
        checkbox = QtWidgets.QCheckBox()
        checkbox.setChecked(False)
        self.rpbandsTable.setCellWidget(row, 0, self._center_widget(checkbox))

        # Name
        name_item = QtWidgets.QTableWidgetItem(name)
        name_item.setTextAlignment(Qt.AlignCenter)
        self.rpbandsTable.setItem(row, 1, name_item)

        # Min freq
        min_item = QtWidgets.QTableWidgetItem(f"{float(min_freq):.1f}")
        min_item.setTextAlignment(Qt.AlignCenter)
        self.rpbandsTable.setItem(row, 2, min_item)

        # Max freq
        max_item = QtWidgets.QTableWidgetItem(f"{float(max_freq):.1f}")
        max_item.setTextAlignment(Qt.AlignCenter)
        self.rpbandsTable.setItem(row, 3, max_item)
        ...
        remove_button.clicked.connect(lambda _, r=row: self.rpbandsTable.removeRow(r))
        self.rpbandsTable.setCellWidget(row, 4, self._center_widget(remove_button))

    def _reset_table(self):
        # try:
        #     self.rpbandsTable.cellChanged.disconnect()
        # except TypeError:
        #     pass
        self._setup_table()
        # self.rpbandsTable.cellChanged.connect()
    ...
    def _accept_and_close(self):
        ...
        for row in range(self.rpbandsTable.rowCount()):
            checkbox_container = self.rpbandsTable.cellWidget(row, 0)
            ...
            name_item = self.rpbandsTable.item(row, 1)
            min_item = self.rpbandsTable.item(row, 2)
            max_item = self.rpbandsTable.item(row, 3)
        ...
        if self.parameters_widget:
            self.parameters_widget.update_band_rp_label(self.accepted_rp_bands)
        self.close()
